...
class RNCWebViewManagerImpl {
    ...
    fun setSource(viewWrapper: RNCWebViewWrapper, source: ReadableMap?, newArch: Boolean = true) {
        ...
        val url = computeUri(view.context, source.getString("uri") ?: "") ?: return
  ...
  fun computeUri(context: Context, sourceUri: String): String? {
    return try {
      val uri = Uri.parse(sourceUri)
      // Verify scheme is set, so that relative uri (used by static resources) are not handled.
      if (uri.scheme == null) computeLocalUri(context, sourceUri)?.path else sourceUri
    } catch (e: Exception) {
      computeLocalUri(context, sourceUri)?.path
    }
  }

  fun computeLocalUri(context: Context, sourceUri: String): Uri? {
    val resId = context.resources.getIdentifier(sourceUri, "raw", context.packageName)
